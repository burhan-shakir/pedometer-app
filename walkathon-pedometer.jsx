...
import rahat_logo from "./src/assets/rahat_logo.png";
...
const firebaseConfig = {
  ...
  projectId: "walkathon-ed707",
  storageBucket: "walkathon-ed707.firebasestorage.app",
  messagingSenderId: "906845305431",
  appId: "1:906845305431:web:8f991d7c5f39d016274112",
  measurementId: "G-L85MFXSFQM"
};
// ──────────────────────────────────────────────────────────────────────────────
...
const MEDALS = ["🥇", "🥈", "🥉"];
const SESSION_KEY = "walkathon_walk_state";
...
export default function App() {
  // Initialise state from sessionStorage immediately so there's no flash to home
  ...
  const [finalData, setFinalData] = useState(saved?.finalData ?? null);
  ...
  const stepsRef = useRef(saved?.steps ?? 0);
  ...
  // ── Persist session on every steps/elapsed change while walking ──────────────
  useEffect(() => {
    if (screen === "walking") {
      saveSession({ screen, name, steps, elapsed });
    }
  }, [screen, name, steps, elapsed]);

  // ── On mount: if a walk was in progress, resume timer + sensors ──────────────
  useEffect(() => {
    if (screen === "walking") {
      // Resume the timer from saved elapsed value
      timerRef.current = setInterval(() => {
        setElapsed(s => s + 1);
      }, 1000);
      // Re-request motion (user gesture already happened before the refresh)
      startListening().then(ok => { if (!ok) setMotionAvailable(false); });
      acquireWakeLock();
    }
  ...
  const handleStart = async () => {
    ...
    setSteps(0);
    setElapsed(0);
    saveSession({ screen: "walking", name, steps: 0, elapsed: 0 });
    setScreen("walking");
  ...
  const saveToLeaderboard = useCallback(async (walkerName, walkerSteps, walkerTime) => {
    setLbLoading(true);
    try {
      const docRef = doc(db, "leaderboard", walkerName.toLowerCase());
      const snapshot = await getDocs(collection(db, "leaderboard"));
      const existing = snapshot.docs.find(d => d.id === walkerName.toLowerCase());
      const existingSteps = existing ? existing.data().steps : 0;
      if (walkerSteps >= existingSteps) {
        await setDoc(docRef, { name: walkerName, steps: walkerSteps, time: walkerTime, updatedAt: Date.now() });
      }
      const finalSnap = await getDocs(collection(db, "leaderboard"));
      const entries = finalSnap.docs.map(d => d.data()).sort((a, b) => b.steps - a.steps);
  ...
  const handleEnd = async () => {
    ...
    const fd = { steps: finalSteps, time: finalTime, dist, cal };
    setFinalData(fd);
    setScreen("summary");
    ...
    <div className="app">
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .logo{display:flex;align-items:center;justify-content:center;gap:10px;font-family:'Bebas Neue',cursive;font-size:15px;letter-spacing:3px;color:rgba(255,255,255,0.4);text-align:center;margin-bottom:32px;}
        .logo-img{width:clamp(32px, 6vw, 42px);height:clamp(32px, 6vw, 42px);object-fit:contain;}
        ...
        .resume-banner{font-size:12px;color:rgba(99,200,255,0.8);text-align:center;margin-top:16px;padding:10px 14px;background:rgba(99,168,255,0.07);border-radius:10px;border:1px solid rgba(99,168,255,0.2);line-height:1.5;}
        ...
        .lb-row{display:flex;align-items:center;gap:12px;padding:11px 14px;border-radius:12px;margin-bottom:6px;background:rgba(255,255,255,0.04);animation:fadeInUp 0.3s ease both;}
      ...
      {screen === "home" && (
        <div className="card">
          <div className="logo">
            <img src={rahat_logo} alt="Miles For Heart Logo" className="logo-img" />
            <div>MILES <span>FOR </span>HEART</div>
            <img src={heart_stroke_found_logo} alt="Heart and Stroke Foundation Logo" className="logo-img" />
        </div>
          <h1>Ready to Walk?</h1>
        ...
        <div className="card">
          ...
          {motionAvailable && !permissionDenied && (
            <div className="notice" style={{color:"rgba(100,220,150,0.7)",background:"rgba(100,220,150,0.05)",borderColor:"rgba(100,220,150,0.15)"}}>
              ✓ Screen will stay on — keep your phone in your pocket or hand
            </div>
          )}
          {saved?.screen === "walking" && (
            <div className="resume-banner">↩ Walk resumed — your previous steps have been restored</div>
          )}
        </div>
      )}
      ...
      {screen === "summary" && finalData && (
        <div className="card">
          ...
          {lbLoading ? (
            <div className="rank-banner"><div className="rank-label">Saving to leaderboard…</div></div>
          ) : myRank && (
            <div className={`rank-banner${myRank === 1 ? " top1" : myRank === 2 ? " top2" : myRank === 3 ? " top3" : ""}`}>
              ...
              <div className="rank-label">{myRank === 1 ? "You're leading the pack!" : `You're #${myRank} on the leaderboard`}</div>
            </div>
          )}
          ...
          <div className="lb-section">
            ...
            {!lbLoading && leaderboard.length === 0 && <div className="lb-loading">No entries yet</div>}
            {!lbLoading && leaderboard.slice(0, 10).map((entry, i) => {
              const isMe = entry.name.toLowerCase() === name.toLowerCase();
              return (
                <div key={entry.name} className={`lb-row${isMe ? " me" : ""}`} style={{animationDelay:`${i*0.05}s`}}>
          ...
          <div className="event-badge"><span className="dot"/>Miles for Heart Charity Walk 2026<span className="dot"/></div>
        </div>
      )}
    </div>
  );
